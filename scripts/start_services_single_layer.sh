...
start_proc() {
  ...
  nohup env SECKILL_CONFIG_PATH="${BASE_CFG}" "${bin}" >"${log_file}" 2>&1 &
  local pid=$!
  echo "${pid}" >"${pid_file}"
...
kill_by_pidfile "${RUN_DIR}/layer.pid"
kill_by_pidfile "${RUN_DIR}/admin.pid"
kill_by_pidfile "${RUN_DIR}/proxy.pid"

echo "[2.5/4] cleanup nacos stale layer instances"
cleanup_nacos_layer_instances

echo "[3/4] start services"
start_proc "layer" "${BIN_DIR}/layer" "${LOG_DIR}/layer.log" "${RUN_DIR}/layer.pid"
start_proc "admin" "${BIN_DIR}/admin" "${LOG_DIR}/admin.log" "${RUN_DIR}/admin.pid"
start_proc "proxy" "${BIN_DIR}/proxy" "${LOG_DIR}/proxy.log" "${RUN_DIR}/proxy.pid"

echo "[4/4] health checks"
wait_health "layer" "http://127.0.0.1:8081/healthz"
wait_health "admin" "http://127.0.0.1:8082/healthz"
wait_health "proxy" "http://127.0.0.1:8080/healthz"

echo "all services started (single layer)"
